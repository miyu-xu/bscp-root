...
set "LOG_DIR=C:\workspace\bscp\bscp\out\dist\logs\initramfs"
mkdir "%LOG_DIR%" 2>nul

echo ================================================
echo  ChromeOS Initramfs + AutoLogin GPU Test - %date% %time%
echo ================================================

"%CROSVM_DIR%\crosvm.exe" --log-level warn run-mp --disable-sandbox --cid 200 --mem 4096 --cpus 2 --no-balloon --no-usb --serial type=file,path=%LOG_DIR%\serial.txt,hardware=serial,num=1,earlycon=true --serial type=sink,hardware=serial,num=2 --serial type=file,path=%LOG_DIR%\hvc.txt,hardware=virtio-console,num=1,console=true --block path=%DISK%,ro=false,lock=false,sparse=false --initrd %INITRD% --params "root=/dev/vda3 rw loglevel=7 console=ttyS0,115200n8 init=/sbin/init rootwait cros_debug" --gpu backend=gfxstream,width=1024,height=768,angle=true,vulkan=true,wsi=vk %KERNEL% 2>%LOG_DIR%\stderr.txt
