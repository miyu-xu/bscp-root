...
if [[ -z "$SERVICE_ROOT" ]]; then
    SERVICE_ROOT="$REPO_ROOT/out/dist/logs/macos-virtmgr-service"
fi
if [[ -z "$LOG_DIR" ]]; then
    LOG_DIR="$REPO_ROOT/out/dist/logs/macos-vm-shell-$(date +%Y%m%d-%H%M%S)"
fi
if [[ -z "$WORK_DIR" ]]; then
...
case "$COMMAND" in
    help)
        cat <<'EOF'
Usage:
  vm_shell_macos.sh -Command start-microdroid [-AutoConnect] [-- extra vm args]
  vm_shell_macos.sh -Command connect [-AdbPort 8035]
EOF
        ;;
    start-microdroid)
        args=(
        ...
        if [[ -n "$APEX_TREE_ROOT" ]]; then
            args+=(-ApexTreeRoot "$APEX_TREE_ROOT")
        fi
