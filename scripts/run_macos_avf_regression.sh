...
run_step_allow_status 124 "run-microdroid" \
    "$VM_WRAPPER" -Command run-microdroid "${VM_COMMON_ARGS[@]}" -LogDir "$OUTPUT_ROOT/run-microdroid" -KeepTemp -TimeoutSecs 120
...
if [[ "$INCLUDE_RUN_APP" -eq 1 ]]; then
    run_step_allow_status 124 "run-app" \
        "$VM_WRAPPER" -Command run-app "${VM_COMMON_ARGS[@]}" -LogDir "$OUTPUT_ROOT/run-app" -KeepTemp -TimeoutSecs 120
...
run_step_allow_status 124 "vm console" \
    "$VM_WRAPPER" -Command console "${VM_COMMON_ARGS[@]}" -LogDir "$OUTPUT_ROOT/console" -PersistVirtmgr -ServiceRoot "$SERVICE_ROOT" -Cid "$CID" -TimeoutSecs 3
