...
:: binder-rpc (CMake) -> copy libs -> cargo virtmgr+vm+crosvm -> dist\windows
...
if "%~1"=="--help" (
    ...
    echo   --clean  Force clean rebuild ^(removes CMake caches and cargo target dirs^)
    exit /b 0
)

if /I not "%RUST_TARGET%"=="x86_64-pc-windows-gnu" (
    echo Error: Windows release builds require RUST_TARGET=x86_64-pc-windows-gnu
    exit /b 1
)
...
if "%CLEAN_BUILD%"=="1" (
    ...
    if exist "%OUT_ROOT%\target" (
        echo Removing root cargo target dir
...
:: Rust binder bindings are generated by bindgen and require libclang.dll. Prefer an explicit
:: caller selection, then system LLVM, then the newest installed Android NDK. Fail here instead
:: of surfacing an opaque panic deep inside the parallel Cargo build.
if not defined LIBCLANG_PATH if exist "%ProgramFiles%\LLVM\bin\libclang.dll" (
    set "LIBCLANG_PATH=%ProgramFiles%\LLVM\bin"
)
if not defined LIBCLANG_PATH if defined LOCALAPPDATA if exist "%LOCALAPPDATA%\Android\Sdk\ndk" (
    for /f "delims=" %%D in ('dir /b /ad /o-n "%LOCALAPPDATA%\Android\Sdk\ndk" 2^>nul') do (
        if not defined LIBCLANG_PATH if exist "%LOCALAPPDATA%\Android\Sdk\ndk\%%D\toolchains\llvm\prebuilt\windows-x86_64\bin\libclang.dll" (
            set "LIBCLANG_PATH=%LOCALAPPDATA%\Android\Sdk\ndk\%%D\toolchains\llvm\prebuilt\windows-x86_64\bin"
        )
    )
)
if not defined LIBCLANG_PATH (
    echo Error: libclang.dll not found. Install LLVM or Android NDK, or set LIBCLANG_PATH.
    exit /b 1
)
call %RUSTC_CMD% -vV | %SystemRoot%\System32\findstr.exe /B /C:"host: x86_64-pc-windows-gnu" >nul
if errorlevel 1 (
    echo Error: Windows release builds require a GNU-hosted Rust toolchain. Current: %RUSTUP_TOOLCHAIN%
    exit /b 1
)
if not exist "!LIBCLANG_PATH!\libclang.dll" (
    echo Error: LIBCLANG_PATH does not contain libclang.dll: !LIBCLANG_PATH!
    exit /b 1
)
...
set "CARGO_TARGET_X86_64_PC_WINDOWS_GNU_AR=%MINGW_BIN%\ar.exe"
cd /d "%REPO_ROOT%"
...
set "COPY_CROSVM=1"
if exist "%DIST_BIN%\crosvm.exe" (
    fc /B "%TGT_OUT%\crosvm.exe" "%DIST_BIN%\crosvm.exe" >nul 2>&1
    if not errorlevel 1 set "COPY_CROSVM=0"
)
if "!COPY_CROSVM!"=="1" (
    ...
    echo crosvm.exe is unchanged; preserving the identical dist file
)
for %%F in (r8Brain.dll ucrtbased.dll vcruntime140d.dll msvcp140d.dll concrt140d.dll) do (
    if exist "%DIST_BIN%\%%F" del /Q "%DIST_BIN%\%%F"
)
for %%F in (libslirp-0.dll) do (
    if exist "%TGT_OUT%\%%F" (
...
for %%F in (libstdc++-6.dll libgcc_s_seh-1.dll libwinpthread-1.dll) do (
    if exist "%MINGW_BIN%\%%F" (
        copy /Y "%MINGW_BIN%\%%F" "%DIST_BIN%\" >nul
    )
)
if "%ENABLE_GFXSTREAM_ANGLE%"=="1" (
    ...
    rem ANGLE_RUNTIME_DIR may be assigned by the subroutine above while this parenthesized block is
    rem already parsed, so every subsequent reference must use delayed expansion.
    ...
    if not exist "!ANGLE_RUNTIME_DIR!\vulkan-1.dll" (
        echo Error: !ANGLE_RUNTIME_DIR!\vulkan-1.dll not found. This ANGLE build uses its bundled Vulkan loader.
        exit /b 1
    )
    ...
    copy /Y "!ANGLE_RUNTIME_DIR!\vulkan-1.dll" "%DIST%\gfx\angle\" >nul
    copy /Y "!ANGLE_RUNTIME_DIR!\libEGL.dll" "%DIST_BIN%\" >nul
    copy /Y "!ANGLE_RUNTIME_DIR!\libGLESv2.dll" "%DIST_BIN%\" >nul
    copy /Y "!ANGLE_RUNTIME_DIR!\vulkan-1.dll" "%DIST_BIN%\" >nul
    if exist "!ANGLE_RUNTIME_DIR!\libGLESv1_CM.dll" (
        copy /Y "!ANGLE_RUNTIME_DIR!\libGLESv1_CM.dll" "%DIST%\gfx\angle\" >nul
    )
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%REPO_ROOT%\scripts\check_angle_runtime.ps1" -RuntimeDir "%DIST%\gfx\angle"
    if errorlevel 1 (
        echo Error: published ANGLE runtime probe failed
        exit /b 1
    )
...
if not exist "%DIST%\THIRD_PARTY_NOTICES" mkdir "%DIST%\THIRD_PARTY_NOTICES"
copy /Y "%REPO_ROOT%\external\crosvm\win_audio\third_party\r8brain\LICENSE" "%DIST%\THIRD_PARTY_NOTICES\r8brain-free-src-LICENSE.txt" >nul
if errorlevel 1 (
    echo Error: failed to stage r8brain MIT license. >&2
    exit /b 1
)
...
>>"%README_FILE%" echo runtime-dlls: libslirp-0.dll libstdc++-6.dll libgcc_s_seh-1.dll libwinpthread-1.dll; r8brain is statically linked into crosvm
>>"%README_FILE%" echo notices: THIRD_PARTY_NOTICES\r8brain-free-src-LICENSE.txt
>>"%README_FILE%" echo lib: libbinder-rpc.dll
if "%ENABLE_GFXSTREAM_ANGLE%"=="1" (
    >>"%README_FILE%" echo gfx-angle wrapper: bin\crosvm-angle.bat
    >>"%README_FILE%" echo gfx-angle runtime: gfx\angle\libEGL.dll gfx\angle\libGLESv2.dll gfx\angle\vulkan-1.dll
)
...
for /r "%ANGLE_ROOT%\out" %%F in (libEGL.dll) do (
    if exist "%%~dpFlibGLESv2.dll" if exist "%%~dpFvulkan-1.dll" (
