   ...
   public:
    ...
    VkResult on_vkEnumeratePhysicalDevices(android::base::BumpPool* pool, VkInstance boxed_instance,
        ...
        if (pPhysicalDeviceCount && pPhysicalDevices) {
            ...
            for (uint32_t i = 0; i < std::min(requestedCount, availableCount); ++i) {
                ...
                uint32_t allTypesMask =
                    (1u << hostMemoryProperties.memoryTypeCount) - 1;
                CoherentHostMemoryProbeResult coherentProbeResult = probeCoherentHostMemory(
                    vk, physicalDevices[i], allTypesMask);
    ...
    VkResult on_vkAllocateMemory(android::base::BumpPool* pool, VkDevice boxed_device,
        ...
        uint32_t coherentHostMemoryTypeMask = 0;
        {
            ...
            coherentHostMemoryTypeMask = physicalDeviceInfo->memoryPropertiesHelper
                ->getCoherentHostMemoryTypeMask();
        }

        // Unified coherent memory enforcement for all allocation paths.
        // If the host supports coherent memory types and the guest requested
        // HOST_COHERENT, the selected memory type must be genuinely coherent.
        const bool guestWantsCoherent =
            memoryPropertyFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
        if (guestWantsCoherent && coherentHostMemoryTypeMask != 0) {
            if (!(coherentHostMemoryTypeMask & (1u << localAllocInfo.memoryTypeIndex))) {
                ERR("vkAllocateMemory: guest requested HOST_COHERENT but "
                    "memory type %u is not in coherent probe mask 0x%x",
                    localAllocInfo.memoryTypeIndex, coherentHostMemoryTypeMask);
                return VK_ERROR_INCOMPATIBLE_DRIVER;
            }
        }
        ...
        if (isExport && hostVisible) {
            if (m_emu->features.SystemBlob.enabled) {
                ...
                // Restrict import-compatible types to genuinely coherent ones
                // if the guest requested coherent. Validation already performed
                // by the unified check above.
                if (memoryPropertyFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT) {
                    memoryHostPointerProperties.memoryTypeBits &= coherentHostMemoryTypeMask;
                }
        ...
        // Mark the backing as genuinely coherent if the host memory type
        // was confirmed by the runtime probe (G6).
        if (coherentHostMemoryTypeMask & (1u << memoryInfo.memoryIndex)) {
            memoryInfo.coherentBacking = true;
        }
    ...
    VkResult vkGetBlobInternal(VkDevice boxed_device, VkDeviceMemory memory, uint64_t hostBlobId) {
        ...
        if (m_emu->features.SystemBlob.enabled && info->sharedMemory.has_value()) {
            ...
            ExternalObjectManager::get()->addBlobDescriptorInfo(
                ...
                std::nullopt, info->coherentBacking);
        } else if (m_emu->features.ExternalBlob.enabled) {
            VkResult result;
            auto device = unbox_VkDevice(boxed_device);
            auto vk = dispatch_VkDevice(boxed_device);
            ...
            ExternalObjectManager::get()->addBlobDescriptorInfo(
                ...
                std::optional<VulkanInfo>(vulkanInfo), info->coherentBacking);
