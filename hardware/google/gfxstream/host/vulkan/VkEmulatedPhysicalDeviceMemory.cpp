...
EmulatedPhysicalDeviceMemoryProperties::EmulatedPhysicalDeviceMemoryProperties(
    ...
    // Coherent memory gating: if a runtime probe has identified genuinely coherent
    // host memory types, only expose HOST_COHERENT for guest types whose mapped host
    // type is in the probe mask. Otherwise, fall back to feature-flag-based gating
    // where HOST_COHERENT requires GlDirectMem or VirtioGpuNext.
