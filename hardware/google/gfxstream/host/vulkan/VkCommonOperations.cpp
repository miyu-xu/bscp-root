...
bool allocExternalMemory(VulkanDispatch* vk, VkEmulation::ExternalMemoryInfo* info,
    ...
    if (sVkEmulation->instanceSupportsMoltenVK) {
        ...
        if (validHandle) {
            ...
            // Phase B diagnostic: check MTLStorageMode to determine if macOS
            // can provide genuinely coherent (MTLStorageModeShared) backing.
            ...
            INFO("Phase B: MTLBuffer storageMode=%ld (%s) for memory type %u (guest-visible "
                 "host type). Is coherent: %s",
                 storageMode, storageModeName,
                 info->typeIndex,
