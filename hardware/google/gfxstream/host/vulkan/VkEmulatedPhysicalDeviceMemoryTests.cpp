...
TEST(VkGuestMemoryUtilsTest, CoherentProbeMatchesHostType) {
    // When the probe finds coherent host types, only guest types mapping
    // to those host types keep HOST_COHERENT.
    const VkPhysicalDeviceMemoryProperties hostMemoryProperties = {
        .memoryTypeCount = 3,
        .memoryTypes =
            {
    ...
    // Probe says only type 0 is coherent. Type 2 (also HOST_COHERENT on
    // the host) is not in the probe mask.
    CoherentHostMemoryProbeResult probeResult;
    probeResult.success = true;
    probeResult.coherentHostMemoryTypeMask = (1u << 0);
    ...
    const VkPhysicalDeviceMemoryProperties expectedGuestMemoryProperties = {
        ...
        .memoryTypes =
            {
                {
                    // Type 0 keeps HOST_COHERENT because it's in the probe mask.
                    .propertyFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                     VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                    .heapIndex = 0,
                },
                {
                    // Type 1 (device local) is unchanged.
                    .propertyFlags = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                    .heapIndex = 1,
                },
                {
                    // Type 2 loses HOST_COHERENT because it's NOT in the probe mask.
                    .propertyFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                     VK_MEMORY_PROPERTY_HOST_CACHED_BIT,
                    .heapIndex = 0,
                },
...
TEST(VkGuestMemoryUtilsTest, CoherentProbeEmptyMaskRemovesCoherent) {
    // When the probe succeeds but returns an empty mask, no guest types
    // keep HOST_COHERENT (assuming GlDirectMem/VirtioGpuNext are not enabled).
    const VkPhysicalDeviceMemoryProperties hostMemoryProperties = {
        .memoryTypeCount = 2,
        .memoryTypes =
            {
...
TEST(VkGuestMemoryUtilsTest, CoherentProbeFailedFallsBackToFeatureGating) {
    // When the probe fails, the code falls back to GlDirectMem / VirtioGpuNext
    // feature gating. Without those features, all HOST_COHERENT is removed.
    const VkPhysicalDeviceMemoryProperties hostMemoryProperties = {
        .memoryTypeCount = 2,
        .memoryTypes =
            {
...
TEST(VkGuestMemoryUtilsTest, CoherentProbePartialMask) {
    // When the probe returns a partial mask, only the matching types keep
    // HOST_COHERENT; other host-coherent types lose it.
    const VkPhysicalDeviceMemoryProperties hostMemoryProperties = {
        .memoryTypeCount = 4,
        .memoryTypes =
            {
    ...
    // Probe says only types 1 and 3 are genuinely coherent.
    CoherentHostMemoryProbeResult probeResult;
    probeResult.success = true;
    probeResult.coherentHostMemoryTypeMask = (1u << 1) | (1u << 3);

    EmulatedPhysicalDeviceMemoryProperties helper(hostMemoryProperties, 2, features,
                                                  probeResult);
    ...
    const VkPhysicalDeviceMemoryProperties expectedGuestMemoryProperties = {
        ...
        .memoryTypes =
            {
                {
                    // Type 0 loses HOST_COHERENT (not in probe mask).
                    .propertyFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
                    .heapIndex = 0,
                },
                {
                    // Type 1 keeps HOST_COHERENT (in probe mask).
                    .propertyFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                     VK_MEMORY_PROPERTY_HOST_COHERENT_BIT |
                                     VK_MEMORY_PROPERTY_HOST_CACHED_BIT,
                    .heapIndex = 0,
                ...
                {
                    // Type 2 (device local) is unchanged.
                    .propertyFlags = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                    .heapIndex = 1,
                },
                {
                    // Type 3 keeps HOST_COHERENT (in probe mask).
                    .propertyFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                     VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                    .heapIndex = 0,
                },
