...
echo Starting ChromeOS with GPU + ChromeOS boot params...
"%CROSVM_DIR%\crosvm.exe" --log-level info run-mp --disable-sandbox --cid 100 --mem 4096 --cpus 2 --no-balloon --no-usb --serial type=file,path=%LOG_DIR%\serial.txt,hardware=serial,num=1,earlycon=true --serial type=sink,hardware=serial,num=2 --serial type=file,path=%LOG_DIR%\hvc.txt,hardware=virtio-console,num=1,console=true --block path=%DISK%,ro=false,lock=false,sparse=false --params "root=/dev/vda3 ro loglevel=7 console=ttyS0,115200n8 noinitrd init=/sbin/init rootwait cros_efi cros_debug cros_norepair" --gpu backend=gfxstream,width=1024,height=768 %KERNEL% 2>%LOG_DIR%\stderr.txt 1>%LOG_DIR%\stdout.txt
